/**
 * motivos-lista.js - Paginación y búsqueda
 */
let g_motivos = [];
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setupEvent('buscarInput', 'input', filtrarMotivos);
    setupEvent('btnClose', 'click', () => window.close());

    setupEvent('btnFirst', 'click', () => seleccionarFila(0));
    setupEvent('btnLast', 'click', () => seleccionarFila(g_motivos.length - 1));
...
async function cargarDatos(pagina = 1) {
    ...
    try {
        ...
        let query = client.from('motivos_baja').select('*', { count: 'exact' });
...
function actualizarPaginadorMotivos() {
    if (typeof window.renderPaginador === 'function') {
        let container = document.getElementById('pg-ui-motivos');
        if (!container) {
            const old = document.querySelector('.pagination-nav');
            if (old) old.style.display = 'none';
            container = document.createElement('div');
            container.id = 'pg-ui-motivos';
...
function seleccionarFila(idx) {
    if (idx < 0 || idx >= g_motivos.length) return;
    g_indexSeleccionado = idx;
    renderTabla();
    const rows = document.querySelectorAll('#tablaMotivos tr');
    if (rows[idx]) rows[idx].scrollIntoView({ block: 'nearest' });
}

function mostrarError(m) {
    const tb = document.getElementById('tablaMotivos');
