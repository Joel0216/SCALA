...
//    articulos                 → id UUID, clave, descripcion, precio, existencia
//    movimientos_inventario    → id SERIAL, numero INTEGER, fecha DATE, hora TIME,
//                                 tipo_id UUID, articulo_id UUID, cantidad, precio_unitario,
//                                 total, observaciones
//
        ...
        data.forEach(t => {
            ...
            tr.innerHTML = `
                ...
                <td style="text-align: center;">
                    <button class="premium-btn btn-danger" style="padding: 4px 8px; font-size: 11px;" onclick="eliminarTipo('${t.clave}')">🗑</button>
                </td>
            `;
...
function limpiarFormTipo() {
    if (G('tipoClave')) G('tipoClave').value = '';
    if (G('tipoDescripcion')) G('tipoDescripcion').value = '';
    if (G('tipoAfecta')) G('tipoAfecta').value = 'SUMA';
    if (G('tipoClave')) G('tipoClave').focus();
}
...
async function guardarTipo() {
    ...
    try {
        // Nuevo tipo siempre (se quitó edición por requerimiento)
        const { error } = await db.from('tipos_movimiento')
            .insert([{ clave, descripcion: desc, afecta_inventario: afecta, activo: true }]);

        if (error) {
            if (error.code === '23505') throw new Error('Esa clave ya existe. Elige otra o cambia la descripción.');
            throw error;
        }
