/**
 * alumnos-lista.js - Standardized Pagination Version
 */
let g_alumnos = [];
...
document.addEventListener('DOMContentLoaded', async () => {
    console.log('Alumnos-Lista: Iniciando...');
    ...
    setupEvent('buscarInput', 'input', filtrarAlumnos);
    setupEvent('btnClose', 'click', () => window.close());

    setupEvent('btnFirst', 'click', () => seleccionarFila(0));
    setupEvent('btnLast', 'click', () => seleccionarFila(g_alumnos.length - 1));
...
async function cargarDatos(pagina = 1) {
    console.log(`Cargando alumnos (Pág ${pagina})...`);
    g_paginaActual = pagina;
    const tbody = document.getElementById('tablaAlumnos');
    if (tbody) tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;">Consultando base de datos...</td></tr>';
    ...
    try {
        ...
        const { data, error, count } = await query
            .order('credencial', { ascending: true })
            .range(from, to);
...
function renderTabla() {
    ...
    if (g_alumnos.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" style="text-align:center; padding:20px;">No hay registros disponibles</td></tr>';
        return;
    }

    g_alumnos.forEach((a, idx) => {
        ...
        tr.onclick = () => {
            ...
            if (window.opener && !window.opener.closed) {
                const functions = ['cargarAlumnoDesdeVentana', 'mostrarAlumno', 'seleccionarAlumno', 'seleccionarAlumnoInterno'];
        ...
        const fechaIng = a.fecha_ingreso ? formatearFecha(a.fecha_ingreso) : 'N/A';
...
function actualizarPaginadorAlumnos() {
    if (typeof window.renderPaginador === 'function') {
        let container = document.getElementById('pg-ui-alumnos');
        if (!container) {
            const old = document.querySelector('.pagination-nav');
            if (old) old.style.display = 'none';
            container = document.createElement('div');
            container.id = 'pg-ui-alumnos';
...
function seleccionarFila(idx) {
    if (idx < 0 || idx >= g_alumnos.length) return;
    g_indexSeleccionado = idx;
    renderTabla();
    const rows = document.querySelectorAll('#tablaAlumnos tr');
    if (rows[idx]) rows[idx].scrollIntoView({ block: 'nearest' });
}

function mostrarError(m) {
    const tb = document.getElementById('tablaAlumnos');
    if (tb) tb.innerHTML = `<tr><td colspan="5" style="text-align:center; color:red;">${m}</td></tr>`;
}
