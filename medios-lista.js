/**
 * medios-lista.js - Paginación y búsqueda
 */
let g_medios = [];
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setupEvent('buscarInput', 'input', filtrarMedios);
    setupEvent('btnClose', 'click', () => window.close());

    setupEvent('btnFirst', 'click', () => seleccionarFila(0));
    setupEvent('btnLast', 'click', () => seleccionarFila(g_medios.length - 1));
...
async function cargarDatos(pagina = 1) {
    ...
    try {
        ...
        let query = client.from('medios_contacto').select('*', { count: 'exact' });
    ...
    g_medios.forEach((m, idx) => {
        ...
        tr.onclick = () => {
            ...
            if (window.opener && !window.opener.closed) {
                const functions = ['cargarDatosDesdeVentana'];
...
function actualizarPaginadorMedios() {
    if (typeof window.renderPaginador === 'function') {
        let container = document.getElementById('pg-ui-medios');
        if (!container) {
            const old = document.querySelector('.pagination-nav');
            if (old) old.style.display = 'none';
            container = document.createElement('div');
            container.id = 'pg-ui-medios';
...
function seleccionarFila(idx) {
    if (idx < 0 || idx >= g_medios.length) return;
    g_indexSeleccionado = idx;
    renderTabla();
    const rows = document.querySelectorAll('#tablaMedios tr');
    if (rows[idx]) rows[idx].scrollIntoView({ block: 'nearest' });
}

function mostrarError(m) {
    const tb = document.getElementById('tablaMedios');
